...
export async function PUT(
  ...
  try {
    ...
    const expense = db.prepare('SELECT * FROM ImpulseExpense WHERE id = ?').get(params.id)
    db.close()
...
export async function DELETE(
  ...
  try {
    const params = await context.params
    const db = getDb()
    db.prepare('DELETE FROM ImpulseExpense WHERE id = ?').run(params.id)
    db.close()
